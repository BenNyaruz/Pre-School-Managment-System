...
{
    public partial class Accounting_Student_Uniform_Payment_Record : Form
    {
        ...
        public Accounting_Student_Uniform_Payment_Record()
        {
            InitializeComponent();
        }

        private void Accounting_Student_Uniform_Payment_Record_Load(object sender, EventArgs e)
        {
            ...
            // TODO: This line of code loads data into the 'system_DatabaseDataSet.Student_Uniform_Payment_tbl' table. You can move, or remove it, as needed.
            //this.student_Uniform_Payment_tblTableAdapter.Fill(this.system_DatabaseDataSet.Student_Uniform_Payment_tbl);


            try
            {
                string Current_School_Term_Query = "Select [School_Term] FROM [Uniform_Managment_tbl]";
            ...
            try
            {
                ...
                Table_Load_command.CommandText = "select * from [Student_Uniform_Payment_tbl] WHERE School_Term = '" + Current_School_Term + "'";
                Table_Load_command.ExecuteNonQuery();
                DataTable dta = new DataTable();
                OleDbDataAdapter adapt = new OleDbDataAdapter(Table_Load_command);
                adapt.Fill(dta);
                student_Uniform_Payment_tblDataGridView.DataSource = dta;
                connection.Close();
            }
            catch (Exception ex)
            {
        ...
        private void View_btn_Click(object sender, EventArgs e)
        {
            if (View_All_radioButton.Checked == true)
            {
                ...
                try
                {
                    ...
                    OleDbCommand Table_Load_command = connection.CreateCommand();
                    Table_Load_command.CommandType = CommandType.Text;
                    Table_Load_command.CommandText = "select * from [Student_Uniform_Payment_tbl]";
                    Table_Load_command.ExecuteNonQuery();
                    DataTable dta = new DataTable();
                    OleDbDataAdapter adapt = new OleDbDataAdapter(Table_Load_command);
                    adapt.Fill(dta);
            ...
            else if (View_Student_radioButton.Checked == true)
            {
                ...
                try
                {
                    ...
                    OleDbCommand Table_Load_command = connection.CreateCommand();
                    Table_Load_command.CommandType = CommandType.Text;
                    Table_Load_command.CommandText = "select * from [Student_Uniform_Payment_tbl] WHERE First_Name = '" + Student_Name_txt.Text.Trim() + "' and School_Term = '" + School_Term_txt_Std.Text.Trim() + "'";
                    Table_Load_command.ExecuteNonQuery();
                    DataTable dta = new DataTable();
                    OleDbDataAdapter adapt = new OleDbDataAdapter(Table_Load_command);
                    adapt.Fill(dta);
        ...
        {
            this.Validate();
            this.student_Uniform_Payment_tblBindingSource.EndEdit();
